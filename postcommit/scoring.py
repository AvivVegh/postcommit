"""postcommit.scoring — cheap, deterministic post-worthiness signals.

No model calls. This is the analysis the SessionEnd hook runs to decide "was
there post-worthy work here?" It reads a session transcript and the repo's git
state and returns a score plus human-readable reasons. Kept separate from the
hook wiring (postcommit.hooks) so it is unit-testable in isolation.
"""
