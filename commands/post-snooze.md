...
Run the postcommit CLI to write the snooze. It operates on the current repo's `.postcommit/state/watermark.json`:

```
postcommit state snooze $ARGUMENTS
```

If `$ARGUMENTS` is empty, run it with no argument (defaults to 3 days).

If `postcommit` is not on PATH (the package isn't installed — install it with `uv tool install postcommit`), fall back to writing the snooze directly:
